...
import {
  ...
  Plus, Minus, ShoppingCart, Sparkles, Star, Truck, Users, UtensilsCrossed,
} from 'lucide-react'
import {
...
import { addonKeys, contact, gallerySources, heroImage, menuItems, serviceKeys, storyImage, whyStandardImage } from './utils/siteData'
...
function StepBasics({ order, update, errors }) {
  ...
  return <div className="step-content"><div><p className="step-number">{b.number}</p><h2>{b.title}</h2><p>{b.copy}</p></div><div className="form-grid">
    ...
    <label>{b.time} <sup>*</sup><input type="time" value={order.time || ''} onChange={e => update('time', e.target.value)} placeholder={b.timePlaceholder} />{errors.time && <Error>{errors.time}</Error>}</label>
    ...
    <label>{b.address} <sup>*</sup><input value={order.address || ''} onChange={e => update('address', e.target.value)} placeholder={b.addressPlaceholder} />{errors.address && <Error>{errors.address}</Error>}</label>
  </div></div>
}
...
function StepReview({ order, items, onEdit, update, errors }) {
  ...
  return <div className="step-content"><div><p className="step-number">{c.order.steps[3]}</p><h2>{r.title}</h2><p>{r.copy}</p></div><div className="review-list">
    ...
    <ReviewGroup title="Cart Items" onEdit={() => {}} rows={items.map(item => [`${item.quantity}x ${item.name}`, item.note ? `Note: ${item.note}` : '-'])} />
